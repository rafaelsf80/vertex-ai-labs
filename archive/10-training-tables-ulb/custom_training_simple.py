# Custom training pipeline, with script located at 'script_custom_training_tf.py"'
...
# OPTIONAL: Create a Managed Dataset from BigQuery. Datasets must be in same location as Vertex project
...
job = aiplatform.CustomTrainingJob(
    display_name="ulb_xgb_custom_training_simple",
    script_path="script_custom_training_tf.py",
    container_uri="europe-docker.pkg.dev/vertex-ai/training/xgboost-cpu.2-1:latest",
    requirements=['google-cloud-bigquery', 'google-cloud-storage', 'db-dtypes'],
    model_serving_container_image_uri="europe-docker.pkg.dev/vertex-ai/prediction/xgboost-cpu.2-1:latest",
)

model = job.run(
    # dataset=ds, OPTIONAL: if you created a Managed Dataset
    model_display_name='ulb-custom-model-simple',
    replica_count=1,
    environment_variables={'bq_dataset': BQ_SOURCE},
